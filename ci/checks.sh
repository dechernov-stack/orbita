...
# Шип 2 (дизайн): капслок в подписях, усечение без подсказки, декоративные
# строки, третья колонка у инженера — список исключений пуст.
echo "== сторожа дизайна ==" ; python3 tools/validate_design_guards.py
...
# Истина 21-09, write_rules.schema_on_write: поле вне схемы вида — отказ. Допуск
