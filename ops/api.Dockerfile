...
# --------------------------------------------------------------------------
# Версия та же, что в тестовых прогонах: разнобой «локально 8.10, в образе
# 8.14.3» — сам по себе дефект, а при плохой сети он ещё и останавливает
# выкат (базовый образ не вытягивается в кэш сборщика).
...
# Живость проверяется СРЕДСТВАМИ ОБРАЗА: bash умеет открыть tcp-сокет через
# /dev/tcp, и curl ради одной проверки ставить не нужно. Заодно сборка
# перестаёт зависеть от apt-репозиториев — на плохой сети это разница между
# «выкат прошёл» и «выкат невозможен» (наблюдение живого прогона).
WORKDIR /opt/orbita
...
#
# Проверять живость модельным маршрутом нельзя: включённые учётки отвечают
# на него 401, и здоровый контейнер объявляется больным (поймано на живом
# стенде, когда владелец включил учётки). /api/auth/whoami открыт по
# определению — он и говорит, включены ли учётки.
HEALTHCHECK --interval=10s --timeout=3s --start-period=40s --retries=6 \
  CMD bash -c 'exec 3<>/dev/tcp/127.0.0.1/8090 \
    && printf "GET /api/auth/whoami HTTP/1.0\r\n\r\n" >&3 \
