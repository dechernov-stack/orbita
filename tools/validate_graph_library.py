...
def main() -> int:
    ...
    if V2.is_dir():
        for файл in sorted(V2.rglob("*.tsx")):
            ...
            # Запрет на свою отрисовку — ГРАФОВЫМ экранам: там рёбра и раскладку
            # считает библиотека, и самодельная картинка разошлась бы со связями.
            # Фигуры продукта (кружок сцены, ромб точки, скошенный квадрат
            # мероприятия) и карта фазы на оси времени — не граф связей: их
            # положение задано ДАТОЙ и дорожкой, раскладывать нечего
            # (ДИЗАЙН-НАПРАВЛЕНИЕ, шип 2). Запрещённым для них остаётся
            # сохранение раскладки — оно ниже, общим правилом.
            if графовый:
                ...
                if "@xyflow/react" not in текст:
                    problems.append(
                        f"{файл.relative_to(ROOT)}: экран графа не берёт @xyflow/react"
                    )
            # Сохранённая раскладка — под запретом у всех экранов v2: координаты
            # либо считаются при показе, либо приходят из данных (дата, дорожка).
            # Браузерная память как таковая законна (выбранный проект, имя
            # автора) — запрещено помнить ПОЛОЖЕНИЕ узлов.
            if re.search(r"сохранить раскладку|saveLayout|onNodeDragStop", текст):
                problems.append(
                    f"{файл.relative_to(ROOT)}: сохранение раскладки — граф вычисляется, не рисуется"
                )
            if графовый and re.search(r"localStorage|sessionStorage", текст):
                problems.append(
                    f"{файл.relative_to(ROOT)}: память раскладки в браузере — координаты не хранятся"
                )
