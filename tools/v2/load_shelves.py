...
# Порождённые полки: наши, не поставка. Сторож отпечатков их не трогает —
# они правятся волнами (шаблон фазы обрастает сценами), и требовать от них
...
ПОРОЖДЁННЫЕ = [
    ...
    # словарь класса миссии — сид генератором из истин (шип 4 §2); термины ложатся принятыми
    ("полки-порождённые/СИД-СЛОВАРЬ.json", "glossary_term"),
    ...
    # класс миссии и его рамки Р1–Р9 (предзаполнение сцены 5 при заведении проекта)
    ("полки-порождённые/ПОЛКА-РАМКИ-Р.json", "constraint"),
    ("полки-порождённые/ПОЛКА-КЛАСС-МИССИИ-НОО-IOT.json", "mission_class"),
]
...
def загрузить() -> int:
    ...
    непринято: list[str] = []
    for имя, вид in ПОЛКИ + ПОРОЖДЁННЫЕ:
        путь = ПОСТАВКА / имя
        if not путь.exists():
            print(f"{вид}: файла поставки нет — пропуск ({имя})")
            continue
        документ = json.loads(путь.read_text(encoding="utf-8"))
        # Файл с `items` — несколько записей одного вида (рамки Р1–Р9): каждая
        # ложится на полку своим кодом; шапка файла — только описание.
        записи = [(з.get("code"), з) for з in документ["items"]] if isinstance(документ.get("items"), list) else [(документ.get("code"), документ)]
        for код, запись in записи:
            if not код:
            ...
            try:
                тело = {"kind": вид, "code": код, "doc": запись, "author": "поставка v2"}
                # статус записей называет сам файл (сид словаря — принятые термины)
                if документ.get("item_status"):
                    тело["status"] = документ["item_status"]
                вызов("POST", "/v2/shelves", тело)
