    ...
    def подготовка_к_точкам(self) -> None:
        ...
        if допущение and строк10 == 0:
            ...
            строк10 = sum(len(э.get("rows", [])) for р in отчёт.get("sections", []) if р.get("no") == "§10"
                          for э in р.get("elements", []))
            if строк10 == 0:
                # Истина 23.09 (assumption_register_rule): §10 читает disposition=assumed, не помету.
                raise Отказ(f"§10 пуст после допущения {факт['id']}: реестр допущений обязан читать диспозицию assumed")
