    ...
    return [
        ...
        раздел("§6", "Технические процессы (NPR 7123.1)",
               [q("6.1", "17 процессов: процесс → механизм «Орбиты» → место", "process_catalog",
                  # Каталог хранится ЗАПИСЯМИ — одна на процесс (истина 21.09); текст
                  # отклонения (tailoring) живёт в notes ядра: своего поля у вида нет.
                  [("Код", "code"), ("Процесс", "name"), ("Группа", "group"), ("Механизм «Орбиты»", "orbita_mechanism"), ("Место", "place"), ("Отклонение (tailoring)", "notes")],
        ...
        раздел("§10", "Инструменты и среда",
               # Инструменты среды у вида «process_catalog» поля не имеют (истина
               # 21.09: запись = процесс): среда — механизм и место каждого процесса,
               # а перечень инструментов пишет человек тезисом по подсказке.
               [q("10.1", "Среда инженерии", "process_catalog",
                  [("Процесс", "name"), ("Механизм «Орбиты»", "orbita_mechanism"), ("Место", "place")],
                  area="library", min_rows=1)],
               "SRR", scenes=["A2"], statements=1, hint="инструменты среды по полке: Управление требованиями и моделью — «Орбита»: реестры, матрицы, состав, история версий; MBSE-модель — сама система: объекты модели и связи между ними, а не отдельный файл; Расчёты — свои расчёты: карты спроса и ёмкости, покрытие, сравнение построений; баллистика — Orekit в резерве, на стенде не применён; Документы — генерация по шаблонам полки со вставками из модели, печать docx и PDF; Справочники — единицы и глоссарий на полке; правка единицы поднимает отпечатки разборов; Служба ИИ — закрытый контур пакетами и внешний контур выгрузкой знаний с отпечатком; свои расчёты — фактом, Orekit — в резерве", source="полка процессов СИ"),
