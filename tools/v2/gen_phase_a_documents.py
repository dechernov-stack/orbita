...
def semp():
    return [
        раздел("§1", "Назначение и область применения",
               # Поля — ИМЕНАМИ ИСТИНЫ: у вида «проект» нет ни «standard», ни
               # «phase» — фаза зовётся `phase_current`, а стандарт живёт фазой,
               # а не проектом (сторож печати, 21.09).
               [q("1.1", "Проект", "project", [("Проект", "name"), ("Класс миссии", "mission_class"), ("Фаза", "phase_current")])],
        ...
        раздел("§6", "Технические процессы (NPR 7123.1)",
               [q("6.1", "17 процессов: процесс → механизм «Орбиты» → место", "process_catalog",
                  [("Код", "code"), ("Процесс", "name"), ("Группа", "group"), ("Механизм «Орбиты»", "orbita_mechanism"), ("Место", "place"), ("Отклонение разрешено", "tailoring_allowed")],
                  area="library", min_rows=17)],
        ...
        раздел("§10", "Инструменты и среда",
               # Среда инженерии — тот же каталог процессов: механизм «Орбиты»
               # и место, где он живёт. Разворота «tools» у вида нет и не было.
               [q("10.1", "Среда инженерии", "process_catalog",
                  [("Процесс", "name"), ("Механизм «Орбиты»", "orbita_mechanism"), ("Место", "place")],
                  area="library", min_rows=1)],
