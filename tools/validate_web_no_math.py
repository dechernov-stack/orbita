...
LAYOUT_ALLOWED = [
    ...
    # Раскладка схемы процесса: координаты dagre — пиксели, а не величины
    # модели. Их считает библиотека при каждом показе, и на сервере их нет
    # (сторож графа это же и требует: координат в модели не хранить).
    ('у.position.y', 'координата раскладки — пиксели библиотеки, не величина модели'),
    ('дела.filter((д) => д.state', 'номер мероприятия в степпере — разметка, не правило'),
]
