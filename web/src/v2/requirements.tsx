...
function TableRow({ т, открыта, onToggle, все }: {
  т: RequirementRow; открыта: boolean; onToggle: () => void; все: RequirementRow[]
}) {
...
/** «Что заденет» — по носителю и по источнику: до правки, а не после. */
function Impact({ т, все }: { т: RequirementRow; все: RequirementRow[] }) {
  const соседи = все.filter((д) => д.code !== т.code && д.carrier && д.carrier === т.carrier)
  const дети = все.filter((д) => д.sources.includes(т.code))
  if (соседи.length === 0 && дети.length === 0) {
    return <div className="v2-dim">ничего: требование пока ни с чем не связано</div>
  }
  return (
    <>
      {дети.length > 0 && <div>выведены из него: {дети.map((д) => д.code).join(', ')}</div>}
      {соседи.length > 0 && <div>на том же носителе: {соседи.map((д) => д.code).join(', ')}</div>}
      <div className="v2-dim">после правки эти связи станут подозрительными — их придётся подтвердить.</div>
    </>
  )
}
