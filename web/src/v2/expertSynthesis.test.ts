...
describe('старый синтез — только эксперт-режим', () => {
  it('кнопка синтеза из среза стоит под флагом expert', () => {
    ...
    const перед = кусок.slice(0, кнопка)
    expect(перед.lastIndexOf('{expert && (')).toBeGreaterThan(передИли(перед))
  })
...
/** Позиция последнего закрытия условной ветки перед кнопкой: флаг обязан быть ближе. */
function передИли(текст: string): number {
  return Math.max(текст.lastIndexOf(')}'), 0)
}
