...
describe('Phase A: сцена открывает реестр с отбором, а не новый экран', () => {
  ...
  it('A6 — одна Концепция, «Модели» вкладкой в контексте фазы A; A7 — технологии; A8 — реестр рисков; A9 — ОСЗ; A10 — стоимость', () => {
    // Шип 5 §7: «Модели» — вкладка самой Концепции; второй строки вкладок над ней нет.
    expect(поверхности).toContain("if (ключ === 'A6') return <Concept project={project} />")
    expect(поверхности).not.toContain('<Вкладки label="контекст сцены A6"')
