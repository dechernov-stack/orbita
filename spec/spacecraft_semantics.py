...
def _run_checks():
    ...
    # Физичные параметры LoRa SF12: порог SNR демодулятора CSS ≈ −20 дБ,
    # полоса 125 кГц, скорость 250 бит/с → Eb/N0 = −20 + 10·lg(125000/250) ≈ +6,3 дБ.
    # Значение приходит из адаптера протокола (TZ-NET-001), здесь оно лишь воспроизведено.
    up = dict(eirp_dbw=-16.0, alt_km=550, freq_hz=868e6, g_over_t_dbk=-15.0,
              bitrate_bps=250, required_ebn0_db=6.3)      # терминал 14 дБм → КА
    ...
    dead = service_elevation_deg(3.0, min_elev_deg=5.0, **{**up, 'eirp_dbw': -45.0})
    ...
    check("при избыточном запасе ограничивает геометрия",
          limiting_factor(service_elevation_deg(3.0, min_elev_deg=5.0,
                          **{**up, 'eirp_dbw': 0.0}), 5.0) == 'geometry')
    check("значение geometry присутствует в схеме service-zone",
          'geometry' in __import__('json').load(open('schemas/contracts/service-zone.schema.json',
              encoding='utf-8'))['properties']['boundary']['properties']['limiting_factor']['enum'])
