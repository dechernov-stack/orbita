...
class ProposalScreening(private val quality: QualityControl = QualityControl()) {
    ...
    fun issues(item: JsonNode, ctx: ScreeningContext = ScreeningContext()): List<String> {
        ...
        // verification_semantics: описан подход, указано средство, есть критерий
        // успеха, описание не пересказывает требование.
        // Правила вызываются БЕЗУСЛОВНО. Ограждение «если блок верификации есть»
        // и было бы той самой облегчённой проверкой: рукописное требование без
        // верификации получает замечание, предложение ИИ — проходило бы молча.
        issues += verificationIssues(item).map { "верификация: $it" }
