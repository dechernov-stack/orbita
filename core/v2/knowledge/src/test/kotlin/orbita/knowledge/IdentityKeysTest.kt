...
class IdentityKeysTest {
    ...
    /**
     * Ключ нужды по истине схем — ядро формулировки. Сторона в ключ не
     * входит: одна формулировка — одна нужда, носителей много (24.09).
     */
    private val поляНужды = listOf("statement_core")
    ...
    fun `ключ называет поля, из которых сложился`() {
        ...
        assertEquals(listOf("statement"), ключ?.fields, "находка обязана сказать, чем именно сравнивали")
    }
    ...
    fun `ключ нужды берётся из онтологии, а не из второго перечня в коде`() {
        ...
        assertEquals(
            ...
            "перечень полей ключа живёт в истине схем",
        )
    }
    ...
    @Test
    fun `та же формулировка у другой стороны даёт тот же ключ — вторая сторона становится носителем`() {
        // Одна формулировка — одна нужда, носителей много (истина 24.09):
        // 43 копии по сторонам на ПМИ-7 и были этой ошибкой.
        val наша = ключи.of(поляНужды, нужда("SH-0001", "Связь в Арктике"))
        val чужая = ключи.of(поляНужды, нужда("SH-0002", "Связь в Арктике"))
        assertEquals(наша?.value, чужая?.value, "сторона — носитель нужды, а не часть её ключа")
    }

    @Test
    fun `у нужды без формулировки ключ не складывается`() {
        val безФормулировки = mapper.createObjectNode().put("stakeholder", "SH-0001")
        assertNull(
            ключи.of(поляНужды, безФормулировки),
