...
class ResearchTaskTest {
    ...
    fun `промпт несёт три блока и перечисляет принятое поле`() {
        ...
        assertTrue(текст.contains("уверенность 0–1"), "у каждой строки ответа — источник, дата, метка, уверенность")
    }

    /**
     * Разметка полей ответа — то, на чём остановился ПМИ-6.
     *
     * Одна колонка «утверждение» на все классы вернула текст роли стороны
     * утверждением, и синтез сделал из него нужду; субъект нёс класс строки
     * («акт»), и четыре разных указа сошлись бы в одну карточку. Оба правила
     * держатся здесь строкой: промпт — единственное место, где внешнему
     * контуру про них говорят.
     */
    @Test
    fun `требования разводят строку стороны и требуют имени предмета в субъекте`() {
        val задача = исследование.formulate(проект, ResearchTrigger.Scene("3"), автор)

        val текст = исследование.prompt(проект, задача.id)

        assertTrue(
            текст.contains("класс · субъект · роль · интерес · масштаб · источник"),
            "у строк класса «сторона» своя разметка: роль и интерес — в своих колонках",
        )
        assertTrue(
            текст.contains("класс · субъект · утверждение · величина · источник"),
            "у остальных строк — утверждение и величина",
        )
        assertTrue(
            текст.contains("Роль стороны — не утверждение и не нужда"),
            "правило сказано словами, а не разметкой: из текста роли синтез сделал нужду",
        )
        assertTrue(
            текст.contains("Субъект — ИМЯ предмета строки, а не её класс"),
            "понятия опознаются по субъекту: класс в субъекте схлопнет все нормы в одну карточку",
        )
    }
