...
class ResearchResultTest {
    ...
    /**
     * Парная проверка к воротам приёма плана: раз сомнительное пакетом не
     * принимается, подтверждение обязано открывать дорогу. Ранг живёт у КАЖДОГО
     * факта своей копией, и срез постановки смотрит именно на неё
     * (`Synthesizer.вСрез`): подними одну карточку — и подтверждённое
     * исследование останется за порогом навсегда.
     */
    @Test
    fun `подтверждение источников поднимает ранг и у фактов материала`() {
        val задача = запущенная()
        val материал = assertNotNull(исследование.result(проект, задача, "ответ.md", ОТВЕТ, автор).resultMaterial)
        разобрать(материал)
        ...
        val после = знания.facts(проект).filter { it.material == материал }
    ...
    @Test
    fun `повторное подтверждение источников версий не плодит`() {
        val задача = запущенная()
        val материал = assertNotNull(исследование.result(проект, задача, "ответ.md", ОТВЕТ, автор).resultMaterial)
        разобрать(материал)
        val коды = фактыРезультата(материал)
        исследование.confirmSources(проект, задача, коды, автор)
        val версии = коды.associateWith { assertNotNull(store.byCode(область, it)).version } +
            (материал to assertNotNull(store.byCode(область, материал)).version)

        исследование.confirmSources(проект, задача, коды, автор)

        версии.forEach { (код, версия) ->
            assertEquals(
                версия, assertNotNull(store.byCode(область, код)).version,
                "то же подтверждение — та же запись: второе нажатие новой версии «$код» не заводит",
            )
        }
    }
