...
class ReconcileManualInputTest {
    ...
    @Test
    fun `сторона под синонимом словаря — дубль принятой по ключу, а не по смыслу`() {
        // Дедупликация сторон — через словарь (истина `glossary_rule`): полное имя
        // службы и её короткое имя — один термин, значит один ключ идентичности.
        store.create(
        ...
        сторона("SK-0001", "Ространснадзор")
        val кандидат = Candidate(
            "c1", "stakeholder",
            // роль та же, что у принятой стороны: проверяется ключ имени, а не противоречие ролей
            mapper.createObjectNode().put("name", "Федеральная служба по надзору в сфере транспорта").put("role", "заказчик"),
        )
        val запуск = сверка.preview(проект, listOf(кандидат), автор, роль)
        val предмет = запуск.items.single()
        // Нового у кандидата нет — второй источник подтверждает принятую сторону;
        // суть проверки — дубль узнан КЛЮЧОМ через словарь, не порогом близости.
        assertTrue(предмет.verdict in setOf(Verdict.AUGMENT, Verdict.CONFIRM), "та же сторона иными словами: ${предмет.findings}")
        val дубль = предмет.findings.single { it.question == Question.DUPLICATE }
        assertEquals("SK-0001", дубль.target)
        assertEquals(Match.KEY, дубль.match, "дубль узнан словарём — ключом, не порогом близости")
    }
