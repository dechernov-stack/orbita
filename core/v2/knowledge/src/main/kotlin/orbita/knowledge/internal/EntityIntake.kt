...
class EntityIntake(
    ...
    override fun accept(project: String, task: String, chosen: List<Int>, author: String): Intake.Accepted {
        ...
        действия.forEachIndexed { i, действие ->
            ...
            // Интересы стороны — список с цитатой (истина 24.09): фраза плана
            // ложится интересом, цитата и основание — от факта действия.
            if (вид == "stakeholder") {
                val фактОснования = основание.takeIf { it.isNotBlank() }?.let { store.byCode(область, it) }
                orbita.kernel.schema.Interests.нормализовать(
                    содержимое, прежние = прежний?.doc,
                    цитата = фактОснования?.doc?.path("quote")?.asText("")?.ifBlank { null },
                    факт = фактОснования?.code,
                )
            }
    ...
    private fun отказПлана(
        ...
        обязательства(понятие).forEach { (поле, чего) ->
            ...
            // требовать имени онтологии от плана значило бы отбивать то,
            // что план назвал правильно (поймано живым прогоном 14.09).
            val имена = listOfNotNull(поле, ссылкиПлана[вид]?.поле?.takeIf { ссылкиПлана[вид]?.вид == поле })
            if (имена.none { имя -> payload.path(имя).asText("").isNotBlank() || !payload.path(имя).isEmpty }) {
        ...
        корень.path("facts").forEachIndexed { i, ф ->
            ...
            when {
                ...
                ключ in уже -> {
                    ...
                    // текстом, у ручного — null, иначе ключ просто отсутствует
                    // и обязательность поля не выполнена.
                    val цитата = ф.path("quote").asText("").trim()
