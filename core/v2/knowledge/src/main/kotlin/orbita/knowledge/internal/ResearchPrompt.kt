...
internal class ResearchPrompt(
    ...
    internal companion object {
        ...
        /**
         * Требования к ответу: что вернуть и КАКИМИ полями; часть отпечатка версии.
         *
         * Разметка строк переложена владельцем после остановки ПМИ-6 (п. 4
         * диагноза `ОСТАНОВКА-ПМИ6-ДИАГНОЗ`). Одна колонка «утверждение» на все
         * классы стоила прогона: текст роли стороны вернулся утверждением, и
         * синтез сделал из него нужду, которой сторона не заявляла. Поэтому у
         * строк класса «сторона» своя разметка полей, а правило сказано словами
         * — формат, который надо угадывать, внешний контур не соблюдает.
         *
         * Второе правило — о субъекте, и в поставке владельца его нет: в
         * материале первого цикла субъект нёс КЛАСС строки («акт»,
         * «программа/стратегия», «объекты применения»), а имя предмета уехало в
         * «утверждение». Понятия опознаются по субъекту (`IdentityKeys` — ключ
         * идентичности из полей онтологии), и четыре разных указа с субъектом
         * «акт» сошлись бы в одну карточку. Поэтому требуем имя предмета, а
         * класс оставляем в его собственной колонке.
         */
        private val ТРЕБОВАНИЯ: String = """
            ...
            - Формат — таблица по каждому вопросу. Первая колонка — КЛАСС строки (сторона ·
              программа · норма · применение · аналог): по нему принятое раскладывается по классам.
              Остальные колонки зависят от класса строки:
              - строки класса «сторона» — `класс · субъект · роль · интерес · масштаб · источник ·
                дата · метка · уверенность`;
              - остальные строки — `класс · субъект · утверждение · величина · источник · дата ·
                метка · уверенность`.
            - **Роль стороны — не утверждение и не нужда.** Роль — чем сторона приходится проекту
              (заказчик · регулятор · оператор · потребитель · поставщик · партнёр); интерес — чего
              она хочет сама. Ни роль, ни интерес не пишутся в колонку «утверждение»: оттуда они
              вернутся нуждой проекта, которой сторона не заявляла.
            - **Субъект — ИМЯ предмета строки, а не её класс.** «Указ Президента РФ от 26.10.2020
              № 645», «План развития СМП до 2035», «порты Приморского края», «Марафон IoT» — и
              никогда «акт», «программа/стратегия», «объекты применения», «аналог»: класс строки уже
              назван первой колонкой. Предметы опознаются по субъекту, и четыре разных акта с
              субъектом «акт» станут одной карточкой вместо четырёх.
