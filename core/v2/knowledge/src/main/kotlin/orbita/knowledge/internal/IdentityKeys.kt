...
internal class IdentityKeys(
    ...
    private fun показательВеличины(величина: JsonNode, field: String): Ключ? {
        ...
        нормализация.размерность(единица)?.let { return Ключ("размерность:$it", listOf("$field.unit")) }
        // Единица вне справочника («сценария», «контракта») — показателем служит
        // она сама: без этого ключ цели не складывался вовсе, и повторный приём
        // заводил восемь целей вторым экземпляром (216, 17.09).
