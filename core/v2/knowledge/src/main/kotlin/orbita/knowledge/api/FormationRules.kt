...
object FormationRules {
     ...
     */
    fun подходит(понятие: Concept, факт: Признаки): Boolean =
        понятие.fromFacts.any { правило -> проверяемое(правило) && совпало(правило, факт) }

    fun подходит(понятие: Concept, факт: JsonNode): Boolean = подходит(понятие, признаки(факт))

    fun подходит(понятие: Concept, факт: Fact): Boolean = подходит(понятие, признаки(факт))
    ...
    private fun совпало(правило: FactRule, факт: Признаки): Boolean {
        ...
        // «Субъект — сторона» проверяется КЛАССОМ, который назвал разбор, а не
        // справочником проекта: правило обязано работать и до того, как сторона
        // заведена, иначе первая же нужда окажется без основания.
        правило.subjectIs?.let { если -> if (факт.entityClass != если) return false }
