// Подсказка происхождения понятия — из онтологии, на НАШЕЙ стороне.
//
// ВОРОТАМИ ЭТО БОЛЬШЕ НЕ СЛУЖИТ. РЕШЕНИЕ-ЧИТАТЬ-СМЫСЛ (владелец, 15.09):
...
object FormationRules {
    ...
    /**
     * ПОХОЖ ли факт на происхождение понятия — подсказка, а не разрешение.
     *
     * Ответ «нет» НЕ значит «нельзя»: с 15.09 предложение не отбивается и не
     * помечается из-за того, что вид факта или предикат не совпали с
     * подсказкой. Пользоваться этим ответом можно только там, где он ничего не
     * запрещает: порядок показа, объяснение человеку, текст промпта.
     */
    /**
