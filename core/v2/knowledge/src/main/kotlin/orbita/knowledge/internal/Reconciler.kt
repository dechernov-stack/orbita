...
internal class Reconciler(
    ...
    override fun preview(
        ...
        // вместо тридцати годных предложений. Непрошедший называется поимённо
        // и уходит в отказ СВОЕЙ строкой — молчать о нём тоже нельзя.
        ...
        if (брак.isNotEmpty()) {
            val список = документ.putArray("refused")
            брак.forEach { список.add(it) }
        }
    ...
    private fun завести(
        ...
        // Одна формулировка — одна нужда (истина 24.09). Копия той же нужды у
        // другой стороны из ТОГО ЖЕ запуска в предпросмотре дубля не видела —
        // принятая первой уже в проекте, и вторую «новую» система не заводит,
        // а прибавляет её сторону носителем: иначе ПМИ-7 снова даст 43 копии.
        if (вид == "need" && уточняет == null && обобщает == null) {
            val ключи = IdentityKeys.of(store, область, shelves)
            val близнец = ключи.ofConcept(понятие.code, снимок)?.let { найти(область, понятие, ключи, it.value) }
            if (близнец != null) {
                val итог = слить(project, область, запись, близнец.code, reason, author)
                return итог.copy(note = "та же формулировка уже принята — ${близнец.code}; ${итог.note}")
            }
        }
        ...
        // Интересы стороны — список её слов с цитатой (истина 24.09): фраза
        // кандидата ложится интересом, цитата и основание — от кандидата-факта.
        if (вид == "stakeholder") {
            orbita.kernel.schema.Interests.нормализовать(
                документ, цитата = кандидат.doc.path("quote").asText("").ifBlank { null }, факт = кандидат.code,
            )
        }
    ...
    private fun кандидатФакт(
        ...
        // Пустой кандидат — брак ОДНОГО предложения, а не всего пакета: до
        // 15.09 одна рамка без формулировки роняла сверку целиком, и человек
        // видел отказ вместо тридцати годных предложений. Причина называется
        // и поля, которые ищутся, — тоже: иначе не понять, что чинить.
        require(текст.isNotBlank()) {
            "кандидат «${словоПонятия(понятие.code)}» без формулировки: " +
                "ни одного из полей ${ПОЛЯ_ФОРМУЛИРОВКИ.joinToString(" · ")} не заполнено — сверять нечего"
        }
    ...
    /**
     * Утверждение кандидата-факта — имя понятия словами: перечней предикатов
     * у истины больше нет (одна истина, 24.09), выдумывать свой нельзя.
     */
    private fun предикатПонятия(понятие: Concept): String = словоПонятия(понятие.code)

    /** Вид кандидата-факта без величины — утверждение: правил по видам фактов нет. */
    @Suppress("UNUSED_PARAMETER")
    private fun видФакта(понятие: Concept): String = "framing"
    ...
    private companion object {
        ...
        /**
         * Поля, в которых у понятия живёт его формулировка. Перечень общий на
