...
internal class DocumentDossier(
    ...
    fun charterGaps(project: String, material: String): GapMap {
        ...
        val пункты = CharterPoints.ПУНКТЫ.map { п ->
            val найденные = блоки.filter { б -> п.есть(б.text) }
        ...
        val заперто = CharterPoints.ВОРОТА_СЦЕН.filter { (_, нужны) -> нужны.any { it in нет } }.keys.toList()
        return GapMap(
            material = material,
            items = пункты,
