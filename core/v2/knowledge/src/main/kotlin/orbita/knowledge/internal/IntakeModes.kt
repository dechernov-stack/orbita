...
internal class IntakeModes(
    ...
    /**
     * Пары фактов группы с РАЗНЫМИ значениями, каждая ровно один раз: у
     * противоречия нет направления, и вторая связь была бы тем же фактом
     * дважды. Порядок пары — по коду факта, чтобы повторный разбор давал ту
     * же связь, а не зеркальную.
     */
    private fun пары(группа: List<Entity>): List<Pair<Entity, Entity>> {
     ...
     * связей дублей не отсеивает.
     *
     * Связь, переставшая быть противоречием, здесь НЕ снимается: снимает её
     * человек диспозицией, а автоматическое снятие унесло бы заодно и
     * `contradicts`, пришедшую из разбора (правило «ИИ не выбирает»).
     */
    ...
    private fun обоснованиеПротиворечия(область: Area, а: Entity, б: Entity): String =
        а.doc.path("predicate").asText("").trim().take(90) +
            ": ${значениеСРангом(область, а)} против ${значениеСРангом(область, б)}" +
            " — показаны оба; ранг подсказывает, победителя выбирает человек"
