...
//
// Обратная сторона того же правила: подтверждение обязано ОТКРЫВАТЬ дорогу.
// Поднимается не только карточка материала, но и все её факты — ранг факта
// проставлен копией при приёме, а смотрят на него и синтез (срез постановки),
// и ворота приёма плана. Поднять одну карточку значит оставить подтверждённое
// исследование за порогом навсегда.
...
internal class ResearchTasks(
    ...
    override fun confirmSources(
        ...
        // Карточка поднимается ПЕРВОЙ: её ранг — тот самый, которым поднимутся
        // факты. Второго значения ранга здесь не появляется: факт берёт ровно
        // то, чем стал его материал.
        val поднят = поднять(область, материал, author)
        val ранг = рангМатериала(область, материал)
        val названные = подтверждённые.map { it.id }.toSet()
        var поднятоФактов = 0
        // Одна правка на факт: свидетельство и ранг ложатся вместе, а факт, в
        // котором не меняется ничего, не переписывается вовсе — повторное
        // подтверждение не плодит версий.
        фактыМатериала(область, материал).forEach { факт ->
            val правка = факт.doc.deepCopy<JsonNode>() as ObjectNode
            var менялось = false
            if (факт.id in названные) {
                ...
                правка.put("evidence", ПОДТВЕРЖДЕНО)
                менялось = true
            }
            ...
            if (менялось) {
                store.update(факт.id, правка, Provenance(Channel.MANUAL, author, source = повод(материал)))
            }
        }
        подтверждённые.forEach { факт -> классФакта(область, факт)?.let { сведено = сведено.plus(it, 1) } }
        val словами = Authority.word(ранг)
        val подъём = when {
            поднят && поднятоФактов > 0 -> "; материал $материал и его факты ($поднятоФактов) подняты до «$словами»"
            поднят -> "; материал $материал поднят до «$словами»"
            поднятоФактов > 0 -> "; факты материала $материал ($поднятоФактов) подняты до «$словами»"
            else -> ""
        }
        val документ = приписка(
            запись.doc,
            "источники подтверждены: фактов ${подтверждённые.size}, $author, ${LocalDate.now()}" + подъём,
            "",
        )
     ...
     *
     * Ранг факта проставлен КОПИЕЙ ранга материала в момент приёма
     * (`EntityIntake`), и подтверждение источников его не трогало: карточка
     * становилась справочной, а факты оставались сомнительными навсегда. Срез
     * постановки смотрит именно на ранг ФАКТА (`Synthesizer.вСрез`) — и
     * подтверждённое исследование в постановку попасть не могло.
     *
     * Экспертный факт не трогается: `expert` весомее `reference`, и «подъём»
     * для него был бы понижением. Пустой ранг (факт заведён до перестройки)
     * тоже не трогается — выдуманный ранг хуже отсутствующего.
     */
    ...
    private fun фактыМатериала(область: Area, материал: String): List<Entity> =
        факты(область).filter { it.doc.path("material").asText("") == материал }
