...
class DocumentReadingTest {
    ...
    @Test
    fun `повторное чтение того же документа оставляет след у каждого понятия`() {
        // Второе чтение записки на 216 (17.09): факты уже лежат, приём их не
        // удваивает — и 50 понятий из 117 остались без следа, то есть без
        // предложения. Повтор обязан находить СВОЙ факт, а не терять его.
        val материал = проект()
        транспорт.ответ = ОТВЕТ
        val первое = читатель.read(ПРОЕКТ, материал, АВТОР)
        assertTrue(первое.proposals.all { it.fact != null })

        val второе = читатель.read(ПРОЕКТ, материал, АВТОР)

        assertEquals(первое.proposals.size, второе.proposals.size, "повтор не теряет понятий")
        val безСледа = второе.proposals.filter { it.fact == null }
        assertTrue(безСледа.isEmpty(), "след есть у каждого и на повторе: ${безСледа.map { it.localId }}")
        assertEquals(
            первое.proposals.map { it.fact }.toSet(), второе.proposals.map { it.fact }.toSet(),
            "следы повтора — те же факты, а не вторые экземпляры",
        )
    }
