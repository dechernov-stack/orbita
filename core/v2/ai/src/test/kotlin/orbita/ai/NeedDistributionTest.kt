...
class NeedDistributionTest {
    ...
    fun `промпт несёт полные перечни и истину о связях, а не правила из кода`() {
        ...
        // Истина 17.09: сервис ждёт не всякая нужда — у стороны-заказчика ждёт.
        assertTrue("ждёт: service" in промпт, "в перечне названо, чем нужда закрывается")
        assertTrue("coverage_expected=service" in промпт, "правило выхода сцены 6 — словами истины")
    }
