...
- ни одна СТОРОНА не осталась без нужды: у каждой есть хотя бы одна;
- общая нужда выписана отдельным пунктом для каждой стороны, которой касается;
...
""".trimIndent()
            ...
            корень.path(массив).forEachIndexed { номер, узел ->
                ...
                if (понятие == "stakeholder") {
                    узел.path("needs").forEachIndexed { н, нужда ->
                        ...
                        пункты += Пункт(имяНужды, "need", сНосителем(нужда, имя))
                    }
                }
            }
        }
    ...
    /** Нужда со ссылкой на свою сторону: носитель у неё ровно один. */
    private fun сНосителем(нужда: JsonNode, сторона: String): JsonNode {
        val узел = нужда.deepCopy<JsonNode>() as ObjectNode
        узел.putObject("stakeholder").put("ref", сторона)
        return узел
    }
    ...
    private fun чтоВыписать(роль: String): String {
        ...
        return buildString {
            ЧТО_ВЫПИСАТЬ.forEach { (код, текст) ->
                ...
                // нехватки» — это `from_facts_note` владельца, и меняется оно
                // правкой YAML, а не правкой кода.
                val понятие = GeneratedOntology.byCode[код]
                понятие?.fromFactsNote?.let { append("\nГде искать: ").append(it).append(".") }
        ...
        val ЧТО_ВЫПИСАТЬ: List<Pair<String, String>> = listOf(
            ...
            "need" to """
                **Нужды — ВНУТРИ каждой стороны, полем `needs`.** Отдельного списка нужд
                ...
                · её собственный интерес из таблицы сторон — ещё одной записью.
