    ...
    private fun чтоВыписать(роль: String): String {
        ...
        return buildString {
            ЧТО_ВЫПИСАТЬ.forEach { (код, текст) ->
                ...
                // Подсказка раздела — тоже истина (`section_hint` правил
                // образования): «§2.1 общие нужды → привязка к сторонам по
                // субъекту/смыслу». Без неё модель не знает, ЧЕЙ раздел читать.
        ...
        val ЧТО_ВЫПИСАТЬ: List<Pair<String, String>> = listOf(
            ...
            "goal" to """
                **Цели** (`goals`). Измеримый результат к сроку: формулировка · значение с
                ...
                постановку не встаёт, она ждёт связи.
            """.trimIndent(),
            "constraint" to """
