...
internal object AnswerSchemas {
    ...
    fun разбор(mapper: ObjectMapper): ObjectNode {
        val факт = объект(
            ...
            обязательные = listOf("kind", "subject", "predicate", "value", "anchor", "mark"),
        )
        ...
        поле(факт, "mark", перечень(mapper, listOf("И", "В", "П"), "метка достоверности"))
        ...
        // Резюме документа для досье (РЕШЕНИЕ-ДОКУМЕНТ-ПЕРВИЧЕН §3): 3–5 строк
        // тем же ответом — о чём документ, что полезного проекту, чего в нём нет.
        поле(корень, "summary", строка(mapper, "3–5 строк: о чём документ, что в нём полезного проекту, чего в нём нет"))
    ...
    private fun риск(mapper: ObjectMapper): ObjectNode {
        ...
        поле(узел, "cec", cec)
        // Мера снижения у модели НЕ спрашивается: истина владельца 17.09 —
        // «measures: ставит человек (сцена 11)»; из документа только
        // формулировка и класс.
        return узел
    }
    ...
    private fun сторона(mapper: ObjectMapper): ObjectNode {
        ...
        //
        // Общая нужда документа названа у каждой стороны, которой касается;
        // в проекте это одна нужда с несколькими носителями (истина 24.09:
        // `owns` n:m) — копии одной формулировки складывает сверка ключом
        // идентичности. Пока нужды шли одним плоским списком, модель писала каждую
