...
РОЛЬ И ИНТЕРЕС СТОРОНЫ — РАЗНОЕ, и это разные виды фактов.
Роль — чем сторона приходится проекту («заказчик», «регулятор
макрорегиона», «оператор мониторинга флота»): факт relation, subject —
сторона, predicate — предикат роли. Из роли нужда не образуется никогда.
Интерес — чего сторона хочет («непрерывность мониторинга на беспилотных
коридорах», «частотные присвоения до запуска»): факт assessment, subject —
та же сторона, метка [П]. Из такого факта нужда-кандидат и образуется.
Одна строка источника «сторона · роль · интерес» даёт ДВА факта: relation
о роли и assessment об интересе, а не один.
$стадияА
## План действий (Д2в)
