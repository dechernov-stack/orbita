...
class Atomizer(
     ...
     */
    /**
     * Роли стороны — перечень истины схем (`stakeholder.role`) русскими
     * метками; перечней предикатов у понятий больше нет (одна истина, 24.09):
     * модель читает смысл, а роль называет из перечня ролей.
     */
    private fun ролиСтороны(): String =
        orbita.kernel.schema.GeneratedKinds.enumLabels["stakeholder.role"]?.values
            ?.joinToString(" · ")
            ?.ifBlank { null }
            ?: "(перечня ролей у истины нет)"

    /** Границы понятия — «чем не является» — словами истины, анти-примером в промпт. */
    ...
    private fun промпт(project: String, material: String, intent: String, блоки: List<CanonBlock>): String {
        ...
        // в промпте нет, и разойтись с истиной ей не с чем.
...
Роль стороны К ПРОЕКТУ называй из перечня ролей истины: $ролиСтороны.
Роли вне перечня не выдумывай — выпиши факт как есть, без роли; предикат
факта — словами источника, пересказом его не порть.
Прямо названную нехватку выписывай фактом framing на того, кому не
хватает, предикат — словами источника.
...
и снимет ненужное. Виды целей и их поля:
  ...
  · stakeholder (сцена 3) — сторона миссии: name · role · interest · influence
    ...
    established)
    Образуется из факта relation о роли К ПРОЕКТУ ($ролиСтороны) либо из
    факта capability об организации. Чем сторона НЕ является: $границыСтороны.
  ...
  · need (сцена 3) — потребность: statement · owner (имя стороны)
    Образуется из факта нехватки (framing) на того, кому не хватает —
    чем нужда НЕ является: $границыНужды; либо
    ...
    факта: одно действие need на каждую сторону-носителя (одна формулировка,
    разные owner) — в проекте это ОДНА нужда с несколькими носителями:
    копий по сторонам система не заводит, одна формулировка — одна нужда.
