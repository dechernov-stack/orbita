...
(«является заказчиком», «регулирует», «является оператором», «поставляет»,
«потребляет»); assessment с меткой [П] — её интерес словами источника.
Прямо названную нехватку («не хватает», «требуется», «нуждается в»)
выписывай фактом framing на того, кому не хватает.
