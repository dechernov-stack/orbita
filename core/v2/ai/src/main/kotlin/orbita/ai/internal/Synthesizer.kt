...
class Synthesizer(
    ...
    private fun отбор(project: String): Отбор {
        ...
        val факты = intake.facts(project)
            ...
            // Урезание сначала по ГОДНОСТИ, потом по рангу. Факт, из которого
            // ни одно понятие онтологии не образуется, синтезу не нужен: он
            // занимает место в потолке и вытесняет тот, из которого сторона
            // или нужда как раз и вышла бы. На поле в 223 факта с потолком 50
            // двенадцать фактов роли попадали в срез случайно — и постановка
            // снова выходила пустой (прогон владельца 15.09).
            //
            // Порядок внутри равных — по коду: иначе отпечаток среза плясал бы
            // от порядка выдачи хранилища и кэш не срабатывал бы никогда.
            .sortedWith(
                compareBy<Pair<Fact, Entity>>(
                    { if (образуетПонятие(it.first)) 0 else 1 },
     ...
     */
    /**
     * Опознаватель субъекта для правила образования: «это сторона проекта?».
     * Оценка об интересе УЖЕ принятой стороны — законное основание нужды, а
     * класс сущности разбор вне режима ТЗ не ставит; без опознавателя такая
     * оценка помечалась бы напрасно. Смотрим в СРЕЗ: в нём и лежит принятое,
     * о котором выносится вердикт, — второго источника заводить незачем.
     */
    private fun знаетСубъекта(slice: Slice): (String, String) -> Boolean = { вид, имя ->
        val искомое = имя.trim().lowercase()
        slice.entities.values.any { сущность ->
            сущность.kind == вид && (
                сущность.code.lowercase() == искомое ||
                    ИМЕНА.any { поле -> сущность.doc.path(поле).asText("").trim().lowercase() == искомое }
                )
        }
    }
    ...
    private fun предложение(
        ...
        val коды = основания(узел)
        // Код ПРИНЯТОЙ сущности в основании — частая ошибка модели: в срезе
        // рядом лежат и факты, и принятое, а основанием бывают только факты
        // (принятое называется полем target). Такое предложение не
        // выбрасывается: человек видит его с пометой и решает сам — шесть
        // предложений из девяти пропадали молча (прогон владельца 15.09).
        val чужие = коды.filter { it !in slice.facts }
        val факты = коды.mapNotNull { slice.facts[it] }
        val пометаОснования = if (чужие.isEmpty()) {
            null
        } else if (чужие.any { it in slice.entities }) {
            "основание ${чужие.joinToString(" · ")} — код ПРИНЯТОЙ сущности, а не факта: " +
                "основанием бывают только факты среза, принятое называется полем «target»"
        } else {
            "основание ${чужие.joinToString(" · ")} вне среза: придуманных кодов фактов не бывает"
        }
        require(факты.isNotEmpty()) {
            пометаОснования ?: "понятие «$код» без факта-основания — предложений без оснований не бывает"
        }
        // Правило образования проверяет ОТВЕТ модели — но синтез ничего не
        // заводит, он предлагает. Поэтому непрошедшее предложение не
        // выбрасывается, а ПОМЕЧАЕТСЯ: человек видит его вместе с причиной и
        // решает сам. Выбрасывать значило бы прятать работу модели и оставлять
        // постановку недособранной без объяснения (прогон владельца 14.09).
        // Жёсткий отказ живёт там, где записи заводятся пакетом, — в приёме
        // плана разбора.
        val помета = if (FormationRules.проверяемо(правило) &&
            факты.none {
                FormationRules.подходит(правило, it, знаетСубъекта(slice), содержимоеДано(узел))
            }
        ) {
        ...
        return FormationProposal(
            ...
            missing = нехватка(правило, содержимое) + listOfNotNull(помета, пометаОснования),
    ...
    /**
     * Поля, названные самим предложением: признак правила («designation»,
     * «year») мог приехать полем, а не текстом факта. Тогда искать его в
     * тексте незачем — он уже назван.
     */
    private fun содержимоеДано(узел: JsonNode): Set<String> =
        узел.path("payload").properties()
            .filter { (_, значение) -> значение.asText("").isNotBlank() || !значение.isEmpty }
            .map { (имя, _) -> имя }
            .toSet()
...
""".trimIndent()
    ...
    /**
     * Образуется ли из факта хоть одно понятие постановки. Проверяется тем же
     * правилом, что стоит воротами, — вторых правил отбора не заводим.
     */
    private fun образуетПонятие(факт: Fact): Boolean =
