...
class Synthesizer(
    ...
    private fun предложение(
        ...
        // Пометы «факт не подходит под правило образования» здесь больше нет.
        // РЕШЕНИЕ-ЧИТАТЬ-СМЫСЛ (15.09): содержательное предложение не
        // отбивается и не порочится по форме — ни видом факта, ни предикатом.
        // На прогоне SR-0027 эта помета висела на трёх сторонах из шести,
        // притом что стороны были названы документом прямо.
        ...
        return FormationProposal(
            ...
            missing = нехватка(правило, содержимое) + listOfNotNull(пометаОснования),
...
""".trimIndent()
    ...
    private fun правилаОбразования(): String = GeneratedOntology.concepts.joinToString("\n") { понятие ->
        buildString {
            ...
            понятие.identity?.let { узнаётся ->
                append("\n    узнаётся по: ${узнаётся.key.joinToString(", ")}")
                append("; по смыслу: ${узнаётся.semantic}")
            }
    ...
    companion object {
        ...
        val ВИДЫ_ПОНЯТИЙ: List<String> = GeneratedOntology.concepts
            // Понятие, которое ложится на ФАКТ (допущение), принятой сущностью
            // не бывает: без этого «принятым» оказывался каждый факт поля и
            // срез синтеза удваивался сам собой.
            .filterNot { it.marksFact }
