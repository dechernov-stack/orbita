...
class NormativeAcceptTest {
    ...
    fun `принятый норматив попадает в карточку поставки, а не заводит двойника`() {
        ...
        // Приём ДОПОЛНЯЕТ карточку: разбор не знает редакции и даты, и они не теряются.
        val док = карточки.first().doc
        assertEquals("действует на 29.08.2026", док.path("edition").asText(), "редакция поставки не стёрта приёмом")
        assertEquals("ПП РФ от 22.12.2020 № 2216", док.path("designation").asText(), "обозначение — то, что полнее")
    }
