...
class SceneSevenEightTest {
    ...
    /**
     * Отказы от объёма читаются экраном и дописываются к решению.
     *
     * §9 отчёта о концепции миссии («Что вынесено за границы первой очереди»)
     * разворачивает `descopes` принятой концепции. Экран их не видел (ответ
     * маршрута их не нёс) и дописать не мог — KDP-A держалась неполнотой
     * документа (владелец, 21.09).
     */
    @Test
    fun `отказы от объёма видны экрану и дописываются к решению`() {
        составБезКонцепции()
        концепция()
        val список = router.handle("GET", "/v2/concept", параметры, null)!!.body.path("items").first()
        assertEquals(
            "межспутниковая связь",
            список.path("descopes").first().path("text").asText(),
            "отказы от объёма приходят экрану: без них §9 нечем наполнить",
        )

        val код = список.path("code").asText()
        router.handle("PATCH", "/v2/entities/$код", параметры,
            """{"fields":{"descopes":[{"text":"межспутниковая связь","impact":"задержка до 6 ч"},
                                      {"text":"собственная наземная сеть","impact":"аренда станций партнёра"}]},
                "author":"Чернов Д."}""")
        val стало = router.handle("GET", "/v2/concept", параметры, null)!!.body.path("items").first()
        assertEquals(2, стало.path("descopes").size(), "дописанный отказ лёг в то же решение")
    }
