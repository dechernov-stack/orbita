...
class ModelRoutes(
    ...
    private fun завестиВариант(проект: String, тело: JsonNode): V2Router.Ответ {
        ...
        val подгруппы = тело.path("subgroups")
        // Показатели нужны СРАВНЕНИЮ, а не самому варианту: §2 отчёта о
        // концепции миссии печатает имя и подгруппы, и требовать показатель
        // ради строки документа значило бы заставлять выдумывать число.
        // Пустой вариант всё же отказывается: без подгрупп и без показателей
        // это имя без содержания.
        require(
            (показатели.isArray && показатели.size() > 0) || (подгруппы.isArray && подгруппы.size() > 0),
        ) {
            "вариант пуст: назовите подгруппы (строй, высота, плоскости) или хотя бы один показатель"
        }
