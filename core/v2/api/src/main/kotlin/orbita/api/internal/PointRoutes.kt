...
class PointRoutes(
    ...
    /**
     * Роли решающего — ровно роли учётки. Без учётки (сборка без входа) ролей
     * нет и отказать некому: допускаются все роли шаблона, и это сказано в
     * ответе движка именем «стенд». За входом учётка есть всегда, и её пустые
     * роли — отказ словами (шип 1, п. 1.1): «все роли» не подставляются.
     */
