...
class SceneRoutes(
    ...
    private fun видДляЭкрана(код: String): V2Router.Ответ {
        ...
        // Те же примечания для показа: коды полей и значений — словами истины
        // (ответ владельца 26.09). Сама истина не меняется: `notes` — как есть.
        val словами = узел.putObject("note_words")
        вид.notes.forEach { (поле, слова) -> словами.put(поле, NoteWords.словами(вид, слова)) }
