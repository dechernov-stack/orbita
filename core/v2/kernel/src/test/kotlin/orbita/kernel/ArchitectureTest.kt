...
class ArchitectureTest {
    ...
    private val порядок = listOf("ядро", "L0", "L1", "L2", "L3", "L4", "L5", "сквозной", "фасад")
    ...
    fun `каркас собран - четырнадцать модулей с объявленным слоем`() {
        val все = модули()
        // 14 доменных модулей плюс фасад api (ТЗ-BACKEND §3, последняя строка карты)
        assertTrue(все.size == 15, "модулей должно быть 15, найдено ${все.size}: ${все.map { it.имя }}")
        все.forEach { м ->
    ...
    fun `зависимости идут только вниз по слоям`() {
        ...
        все.values.forEach { м ->
            ...
            // сквозной (ai) и фасад (api) по построению смотрят вниз во все
            // доменные модули — это их роль, а не нарушение
            if (м.слой == "сквозной" || м.слой == "фасад") return@forEach
    ...
    /**
     * Чужая схема БД читается только через порт владельца
     * (ВОЛНА-0-ПРИНЯТА §1: сборочные файлы такого не видят — нужна проверка
     * по коду). Модуль пишет SQL только к своей схеме `orbita_<модуль>`:
     * запрос в чужую таблицу обходит порт молча, и через полгода схема
     * владельца меняется, а ломается сосед.
     */
    @Test
    fun `модуль не читает чужую схему БД`() {
        val нарушения = mutableListOf<String>()
        val схемаМодуля = Regex("""orbita_([a-z_]+)\.""")
        модули().forEach { м ->
            val исходники = корень.resolve("${м.имя}/src")
            if (!исходники.isDirectory) return@forEach
            исходники.walkTopDown().filter { it.extension == "kt" }.forEach { файл ->
                схемаМодуля.findAll(файл.readText())
                    .map { it.groupValues[1] }
                    .filter { it != м.имя }
                    .distinct()
                    .forEach { чужая ->
                        нарушения += "${м.имя}/${файл.name}: обращение к схеме orbita_$чужая " +
                            "мимо порта модуля «$чужая»"
                    }
            }
        }
        assertTrue(
            нарушения.isEmpty(),
            "схема БД принадлежит модулю; соседи ходят через порт (ТЗ-BACKEND §2.2): $нарушения",
        )
    }
}
