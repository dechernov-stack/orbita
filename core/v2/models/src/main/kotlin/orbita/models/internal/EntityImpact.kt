    ...
    override fun of(project: String, code: String, depth: Int): ImpactView {
        ...
        return ImpactView(
            ...
            summary = if (поВидам.isEmpty()) {
                ...
                // Вид называется ПО-РУССКИ: имя вида есть в истине схем, и
                // «заденет: component — 2» человеку ничего не говорит.
                "заденет: " + поВидам.entries
                    .joinToString(" · ") { (вид, сколько) -> "${имяВида(вид)} — $сколько" }
            },
        )
    }

    /** Имя вида по-русски из перечня схем; неизвестный вид — как есть. */
    private fun имяВида(код: String): String =
        orbita.kernel.schema.GeneratedKinds.byCode[код]?.title ?: код
