...
class GateRefusalTest {
    ...
    /**
     * Решение владельца 12.09 («ЗНАНИЯ-V2-ПРИНЯТЫ» §1): класс обслуживания у
     * нужды обязателен, но значение TBR допустимо — до сцены сервисов. Здесь
     * оно закрывается: нужда, покрытая сервисом, без класса сцену не выпускает.
     * Нужда БЕЗ сервиса это условие не держит — о ней говорит соседнее.
     */
    @Test
    fun `покрытая нужда с классом TBR — отказ, назначенный класс его снимает`() {
        проект()
        val непокрытая = store.create("ND-0002", "need", область, "3",
            mapper.readTree("""{"statement":"нужда без сервиса","qos_class":{"owner":"Иванов И.","gate":"6"}}"""),
            провенанс)
        assertNull(
            оценщик.why(проект, "covered_need_has_qos_class"),
            "нужду без сервиса это условие не держит: ${непокрытая.code}",
        )

        val нужда = store.create("ND-0003", "need", область, "3",
            mapper.readTree("""{"statement":"телеметрия груза в пути","qos_class":{"owner":"Иванов И.","gate":"6"}}"""),
            провенанс)
        val сервис = store.create("SV-0001", "service", область, "6",
            mapper.readTree("""{"name":"короткие сообщения","qos_class":"B′"}"""), провенанс)
        links.link("covers", сервис.id, нужда.id, провенанс)
        val почему = причина("covered_need_has_qos_class")
        assertTrue("TBR" in почему && "телеметрия груза" in почему, почему)

        store.update(нужда.id,
            mapper.readTree("""{"statement":"телеметрия груза в пути","qos_class":"B′"}"""), провенанс)
        assertNull(оценщик.why(проект, "covered_need_has_qos_class"), "класс назначен — условие снято")
    }
