...
internal class Queries(
    ...
    fun rows(project: String, элемент: JsonNode, подписи: Map<String, String> = emptyMap()): List<List<String>> {
        ...
        if (разворот.isNotBlank()) {
            ...
            return все.flatMap { сущность ->
                сущность.doc.path(разворот).map { пункт ->
                    колонки.map { поле ->
                        словом(вид, "$разворот.$поле", человечно(путём(пункт, поле)), подписи)
                    }
                }
            }
        }
        return все.filter { подходит(it, отбор) }
            .map { сущность ->
                колонки.map { поле -> словом(вид, поле, значение(сущность, поле), подписи) }
            }
    }
    ...
    /**
     * Значение перечисления — русской меткой ИСТИНЫ, а не кодом модели.
     *
     * Словарь шаблона плоский: «technical» в нём одно на все виды, а истина
     * знает, что у ограничения это «техническое», а у риска — «техническая».
     * Поэтому метка ищется по ВИДУ и ПОЛЮ (`constraint.type`), и только
     * потом остаётся словарь шаблона. Поле не перечисление или значение вне
     * перечня — текст идёт как есть: чинить молча печать не должна.
     */
    private fun словом(вид: String, поле: String, значение: String, подписи: Map<String, String>): String =
        orbita.kernel.schema.GeneratedKinds.enumLabels["$вид.$поле"]?.get(значение)
            ?: поПодписи(значение, подписи)
    ...
    private fun человечно(узел: JsonNode): String = when {
        ...
        узел.isObject -> {
            ...
            when {
                ...
                // Внешняя система (истина 24.09): именованный объект вне состава —
                // ICD печатает её именем, не парой «имя · владелец».
                узел.path("name").isTextual && узел.path("name").asText("").isNotBlank() -> узел.path("name").asText()
