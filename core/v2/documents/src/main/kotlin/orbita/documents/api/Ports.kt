...
data class SectionView(
    ...
    /**
     * Пустой перечень закрывается ТЕЗИСОМ человека.
     *
     * «Открытые вопросы: 0 из 1» — и владелец спрашивает: «а если их просто
     * нет?» (22.09). Список, который законно бывает пустым, нельзя требовать
     * строкой: правильный ответ документа — «открытых вопросов нет», сказанное
     * словами, с именем и датой. Правило объявляет ШАБЛОН, не код.
     */
...
data class DocumentView(
    ...
    val version: Int = 1,
    /** Базовых линий у документа: ноль — ворота зрелости F его не пустят. */
    val baselines: Int = 0,
    /** Имя последней линии: им документ и уходил на точку. */
